...
public class ModConfigHelper {
    ...
    public static final Path CONFIG_DIR = FMLPaths.getOrCreateGameRelativePath(FMLPaths.CONFIGDIR.get().resolve(MoreMekasuitModules.MOD_NAME));
    ...
    public static void registerConfig(Map<IConfigSpec, IMekanismConfig> knownConfigs, ModContainer modContainer, IMekanismConfig config) {
        modContainer.registerConfig(config.getConfigType(), config.getConfigSpec(), MoreMekasuitModules.MOD_NAME + "/" + config.getFileName() + ".toml");
